...
def _train_epoch_step(
    ...
    Refactoring:
    - Vereinheitlichte Schleife für DataLoader und manuelles Slicing.
    - Integriertes Gradient Clipping zur Vermeidung von NaN bei 2-Bit Quantisierung.
    """
    model.train()
    running_loss = 0.0
    num_batches_processed = 0
    
    # --- 1. Iterator vorbereiten ---
    # Wir erstellen einen Iterator, der immer (inputs, labels) liefert,
    # egal ob die Daten aus einem DataLoader oder Tensoren kommen.
    iterator = None
    total_batches = 0

    if use_loader:
        # Fall A: Standard PyTorch DataLoader
        iterator = data_loader_or_tensor
        # Sicherstellen, dass wir eine Länge für tqdm haben
        try:
            total_batches = len(data_loader_or_tensor) # type: ignore
        except TypeError:
            total_batches = 0 # Fallback für unendliche Loader
    else:
        # Fall B: Manuelles Slicing (als Generator verpackt)
        X_train = data_loader_or_tensor
        y_train = labels_tensor_if_manual
        bs = batch_size_if_manual

        # Validierung
        if y_train is None:
            if logger: logger.error("labels_tensor_if_manual ist None bei manuellem Slicing.")
            return 0.0
        if not isinstance(bs, int) or bs <= 0:
            if logger: logger.error(f"Ungültige batch_size ({bs}) für manuelles Slicing.")
            return 0.0

        # Anzahl der Batches berechnen
        total_batches = (len(X_train) + bs - 1) // bs # type: ignore

        # Generator-Funktion definieren (Lazy Loading)
        def manual_batch_generator(x_t, y_t, batch_size, num_batches):
            for i in range(num_batches):
                start = i * batch_size
                end = start + batch_size
                # Slicing und Yielding
                yield x_t[start:end], y_t[start:end]
        
        iterator = manual_batch_generator(X_train, y_train, bs, total_batches)

    if total_batches == 0:
        if logger: logger.warning("Keine Batches zum Trainieren verfügbar.")
        return 0.0

    # --- 2. Einheitliche Trainingsschleife ---
    progress_bar = tqdm(
        iterator,
        total=total_batches,
    ...
    for batch_data in progress_bar:
        # Daten entpacken (funktioniert jetzt für beide Fälle gleich)
        inputs, labels = batch_data
        
        # Leere Batches überspringen (Sicherheitsnetz für manuelles Slicing)
        if inputs.shape[0] == 0:
            continue
        ...
        # --- WICHTIG: Gradient Clipping ---
        # Verhindert explodierende Gradienten, die zu NaN führen (essentiell für 2-Bit)
    ...
    if num_batches_processed > 0:
        ...
        if logger: logger.warning("Keine Batches in der Epoche erfolgreich verarbeitet.")
        return 0.0
...
def plot_weight_matrix_heatmap(
    ...
    try:
        # Dynamische Anpassung der Figurengröße
        plt.figure(figsize=(
            max(10, weights_or_mask.shape[1] * 0.15), 
            max(8, weights_or_mask.shape[0] * 0.15)
        ))
        
        # Ticks intelligent setzen
        max_ticks = 30
        ...
        sns.heatmap(
            weights_or_mask,
            cmap="viridis",  # 'viridis' (0=dunkel, 1=hell)
            cbar=True,
...
def analyze_neuron_connectivity(
    ...
    """
    Analysiert die Konnektivität und erstellt Heatmaps.
    """
    ...
    for name in layer_fc_names:
        ...
        if current_mask_data is not None:
            ...
            logger_instance.info(f"    Output-Neuronen (Zeilen) komplett geprunt: {dead_output_neurons} ({dead_output_neurons/num_output_neurons:.1%})")
            logger_instance.info(f"    Input-Features (Spalten) komplett geprunt: {dead_input_features} ({dead_input_features/num_input_features:.1%})")
    ...
    if 'fc1' in weights_binary_masks and 'fc2' in weights_binary_masks:
        ...
        num_neurons_h1 = fc1_mask_data.shape[0]
    ...
    if 'fc2' in weights_binary_masks and 'fc3' in weights_binary_masks:
        ...
        num_neurons_h2 = fc2_mask_data.shape[0]
...
def run_qat_training_pipeline(config_path: str = "config.yaml") -> Optional[str]:
    ...
    Diese Version implementiert strukturiertes Pruning (Entfernen von Neuronen)
    zur Laufzeit, um die Architektur für FHE zu optimieren.
    """
    ...
    unpruned_neurons_val = model_cfg.get("unpruned_neurons", None)
    pbt_layers_val = model_cfg.get("pbt_layers", [])
    ...
    log.info(f"PBT Konfiguration: Behalte {unpruned_neurons_val} Inputs in Layern {pbt_layers_val}")

    run_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    dataset_name = run_cfg.get("dataset_name", "UnknownDataset")

    # Basis-Identifikator für Dateinamen (ohne 'unpruned_neurons')
    cfg_h_early = model_cfg.get("n_hidden", "N")
    cfg_qb_early = model_cfg.get("quantization_bits", "N")
    base_run_identifier = f"QAT_{dataset_name}_{cfg_h_early}h_qb{cfg_qb_early}"
    
    # --- Korrigierte Ordner-Logik (Bereits implementiert) ---
    ...
    try:
        ...
        # --- Modell-Initialisierung (ohne altes Pruning) ---
        ...
        # KORREKTUR: Instanziierung mit n_hidden_1 und n_hidden_2
        torch_model = QATPrunedSimpleNet(
            ...
            unpruned_neurons=unpruned_neurons_val,
            pbt_layers=pbt_layers_val
        ).to(device)
        ...
        opt_momentum = optimizer_cfg.get("momentum", 0.9)
        
        if optimizer_name == "adamw": 
            optimizer = optim.AdamW(torch_model.parameters(), lr=learning_rate, weight_decay=opt_weight_decay)
        elif optimizer_name == "adam": 
            optimizer = optim.Adam(torch_model.parameters(), lr=learning_rate, weight_decay=opt_weight_decay)
        elif optimizer_name == "sgd": 
            optimizer = optim.SGD(torch_model.parameters(), lr=learning_rate, momentum=opt_momentum, weight_decay=opt_weight_decay)
        else: 
        ...
        # --- Lade Konfiguration für strukturiertes Pruning ---
        neuron_pruning_cfg = train_cfg.get("neuron_pruning", {})
        pruning_enabled = neuron_pruning_cfg.get("enable", False)
        pruning_run_at_epoch = neuron_pruning_cfg.get("run_at_epoch", -1)
        pruning_done = False # Verfolgen, ob Pruning bereits durchgeführt wurde
        ...
        for epoch_idx in range(num_epochs):
            ...
            # -------------------------------------------------------
            # ITERATIVES STRUKTURIERTES PRUNING (CODE BEGINN)
            # -------------------------------------------------------
            
            # 1. Konfiguration auslesen (mit Defaults falls nicht in Config)
            pruning_steps = neuron_pruning_cfg.get("pruning_steps", 3)     # Default: 3 Schritte
            pruning_interval = neuron_pruning_cfg.get("pruning_interval", 10) # Default: Alle 10 Epochen
            start_epoch = pruning_run_at_epoch
            
            # Berechne die Epochen, an denen geprunt wird (z.B. 20, 30, 40)
            schedule_epochs = [start_epoch + i * pruning_interval for i in range(pruning_steps)]
            
            # Prüfen, ob wir in einer Pruning-Epoche sind
            if pruning_enabled and (actual_epochs in schedule_epochs):
                
                # Bestimmen, welcher Schritt dies ist (1-basiert)
                step_idx = schedule_epochs.index(actual_epochs) + 1
                progress_factor = step_idx / pruning_steps  # z.B. 0.33 -> 0.66 -> 1.0
                
                log.info(f"--- ✂️ Iteratives Pruning Schritt {step_idx}/{pruning_steps} "
                         f"(Ziel-Fortschritt: {progress_factor:.1%}) in Epoche {actual_epochs} ---")
                
                # Definition der Pruning-Kette (Layer-Abhängigkeiten)
                pruning_chain = [
                    (torch_model.fc1, torch_model.fc2, "pruning_ratio_fc1"),
                    (torch_model.fc2, torch_model.fc3, "pruning_ratio_fc2")
                ]
                
                prev_output_mask = None 

                # --- Schritt A: Masken berechnen und anwenden ---
                for module, next_module, ratio_key in pruning_chain:
                    
                    # 1. Ziel-Ratio für diesen Schritt berechnen (Linearer Anstieg)
                    final_target_ratio = neuron_pruning_cfg.get(ratio_key, 0.0)
                    current_step_ratio = final_target_ratio * progress_factor
                    
                    # 2. Eingänge (Spalten) prunen (Kettenreaktion vom vorherigen Layer)
                    if prev_output_mask is not None:
                        log.info(f"    Prune Eingänge (Spalten) von {module._get_name()} basierend auf vorherigem Layer.")
                        # Maske [1, N_in] auf [N_out, N_in] expandieren
                        input_mask_1d = prev_output_mask.view(1, -1)
                        input_mask_expanded = input_mask_1d.expand_as(module.weight)
                        prune.custom_from_mask(module, name="weight", mask=input_mask_expanded)
                        
                    # 3. Ausgänge (Zeilen) prunen (Basierend auf aktueller Ratio)
                    if final_target_ratio > 0:
                        # L2-Norm der Zeilen berechnen
                        # (Bereits geprunte Zeilen haben Norm 0 und bleiben somit geprunt)
                        norm = torch.norm(module.weight, p=2, dim=1)
                        num_neurons = norm.shape[0]
                        k_to_prune = int(num_neurons * current_step_ratio)
                        
                        log.info(f"    Prune {module._get_name()}: Entferne {k_to_prune} von {num_neurons} Neuronen "
                                 f"(Aktuelle Ratio: {current_step_ratio:.4f}).")

                        if k_to_prune > 0 and k_to_prune < num_neurons: 
                            # Threshold bestimmen (k-kleinste Werte)
                            threshold = torch.kthvalue(norm, k=k_to_prune).values
                            
                            # Neue Maske erstellen (1 = behalten, 0 = entfernen)
                            output_mask = (norm > threshold).float().to(device)
                            
                            # Maske auf Gewichts-Dimensionen expandieren [N_out, 1] -> [N_out, N_in]
                            output_mask_expanded = output_mask.view(-1, 1).expand_as(module.weight)
                            
                            # Pruning anwenden
                            prune.custom_from_mask(module, name="weight", mask=output_mask_expanded)
                            
                            # Bias ebenfalls prunen, falls vorhanden
                            if module.bias is not None:
                                prune.custom_from_mask(module, name="bias", mask=output_mask)
                            
                            prev_output_mask = output_mask
                        else:
                            prev_output_mask = None 
                            log.warning(f"    Keine Neuronen in {module._get_name()} zu prunen (k={k_to_prune}).")
                    else:
                        prev_output_mask = None

                # 4. Letzte Kettenreaktion (Eingänge des Output-Layers fc3)
                if prev_output_mask is not None and hasattr(torch_model, 'fc3'):
                    log.info(f"    Prune Eingänge (Spalten) von fc3 basierend auf fc2.")
                    input_mask_expanded = prev_output_mask.view(1, -1).expand_as(torch_model.fc3.weight)
                    prune.custom_from_mask(torch_model.fc3, name="weight", mask=input_mask_expanded)
                
                # --- Schritt B: Masken sofort permanent machen ---
                # Dies ist entscheidend für iteratives Pruning: Die Nullen werden "eingebacken",
                # damit sie im nächsten Schritt korrekt als 0 erkannt werden (Norm=0).
                log.info("  Mache Pruning permanent (fixiere Nullen)...")
                all_modules_to_remove = [torch_model.fc1, torch_model.fc2, torch_model.fc3]
                for module in all_modules_to_remove:
                    if hasattr(module, 'weight_mask'): 
                        prune.remove(module, 'weight')
                    if module.bias is not None and hasattr(module, 'bias_mask'): 
                        prune.remove(module, 'bias')

                # --- Schritt C: Optimizer Reset ---
                # Momentum löschen, damit der Optimizer nicht versucht, tote Neuronen wiederzubeleben
                current_lr = optimizer.param_groups[0]['lr']
                if optimizer_name == "adamw": 
                    optimizer = optim.AdamW(torch_model.parameters(), lr=current_lr, weight_decay=opt_weight_decay)
                elif optimizer_name == "adam": 
                    optimizer = optim.Adam(torch_model.parameters(), lr=current_lr, weight_decay=opt_weight_decay)
                elif optimizer_name == "sgd": 
                    optimizer = optim.SGD(torch_model.parameters(), lr=current_lr, momentum=opt_momentum, weight_decay=opt_weight_decay)
                else: 
                    optimizer = optim.AdamW(torch_model.parameters(), lr=current_lr, weight_decay=opt_weight_decay)
                
                log.info(f"  Optimizer reset durchgeführt. LR beibehalten bei: {current_lr}")
                
                # --- Schritt D: Best-Model Metriken zurücksetzen ---
                # Wir wollen das beste Modell NACH dem vollständigen Pruning finden.
                # Wir setzen es bei jedem Schritt zurück, um sicherzugehen, dass wir kein 
                # halb-gepruntes Modell als "Bestes" speichern.
                best_val_loss = float('inf')
                best_f1_w = 0.0
                log.info("  'Best Model' Metriken zurückgesetzt. Suche nach neuem Optimum beginnt.")

                # Status-Update
                if step_idx == pruning_steps:
                    pruning_done = True
                    log.info("--- ✅ Iteratives Pruning abgeschlossen. Alle Schritte durchgeführt. ---")
            
            # -------------------------------------------------------
            # ITERATIVES STRUKTURIERTES PRUNING (CODE ENDE)
            # -------------------------------------------------------
        ...
        # --- (Rest der Datei: Artefakt-Management, Evaluierung, JSON-Log...) ---
        # (Der Code von Zeile 734 bis 946 bleibt unverändert, aber die 
        #  Variablen-Tippfehler in Zeile 856/857 sind hier korrigiert)
        
        # --- ANFANG ÄNDERUNG 1: BERECHNUNG DER GESAMT-SPARSITY ---
        # (Original-Zeile 734)
        final_pruning_summary = get_pruning_summary(torch_model)
        log.info(f"Finales Pruning Summary (Layer-wise): {final_pruning_summary}")

        # --- NEU: BERECHNUNG DER GESAMT-SPARSITY ---
        total_weights_combined = 0
        total_zeros_combined = 0
        overall_sparsity_calculated = 0.0

        if isinstance(final_pruning_summary, dict):
            for layer_name, stats in final_pruning_summary.items():
                # Stellt sicher, dass 'stats' ein Diktat ist (um Fehler zu vermeiden)
                if isinstance(stats, dict):
                    total_weights_combined += stats.get("total_weights", 0)
                    total_zeros_combined += stats.get("zero_weights", 0)
        
        if total_weights_combined > 0:
            overall_sparsity_calculated = (
                total_zeros_combined / total_weights_combined
            )
            log.info(
                f"Gesamt-Sparsity berechnet: {total_zeros_combined} Nullen / "
                f"{total_weights_combined} Gewichte = "
                f"{overall_sparsity_calculated:.4f}"
            )
        else:
            log.warning(
                "Konnte Gesamt-Sparsity nicht berechnen, da "
                "'total_weights_combined' 0 ist oder "
                "'final_pruning_summary' kein dict war."
            )
        # --- ENDE NEUER BERECHNUNG ---

        if wandb_mode != "disabled" and wandb.run: 
            wandb.summary["final_pruning_summary_layerwise"] = final_pruning_summary
            # NEU: Gesamt-Sparsity auch zu WandB loggen
            wandb.summary["overall_sparsity_calculated"] = overall_sparsity_calculated
        # --- ENDE ÄNDERUNG 1 ---
        ...
        torch_eval_res = {}
        pruning_connectivity_report: Dict[str, Any] = {}
        pytorch_eval_cfg_params = eval_cfg.get("pytorch_model_eval", {})
        run_pytorch_eval = pytorch_eval_cfg_params.get("run_eval", True)
        
        # --- FIX: Dynamische PBT-Entscheidung ---
        # Wir prüfen, ob das gespeicherte state_dict noch Masken enthält.
        # Wenn durch strukturiertes Pruning die Masken schon entfernt wurden ("eingebacken"),
        # dürfen wir das neue Modell NICHT mit PBT initialisieren, sonst erwartet es Masken, die nicht da sind.
        pbt_needed_for_load = False
        if best_model_state:
            for k in best_model_state.keys():
                if "weight_mask" in k:
                    pbt_needed_for_load = True
                    break
        
        # Entscheidung, welche Parameter wir übergeben
        eval_unpruned_neurons = unpruned_neurons_val if pbt_needed_for_load else None
        eval_pbt_layers = pbt_layers_val if pbt_needed_for_load else []
        
        if pbt_needed_for_load:
            log.info("State Dict enthält Masken -> Initialisiere Eval-Modell MIT PBT-Logik.")
        else:
            log.info("State Dict enthält 'eingebackene' Nullen -> Initialisiere Eval-Modell OHNE PBT-Logik.")
        # ----------------------------------------

        if run_pytorch_eval and best_model_state:
            pytorch_eval_n_samples = pytorch_eval_cfg_params.get("n_samples", None);
            
            eval_model = QATPrunedSimpleNet(
                input_size=input_size, 
                num_classes=num_classes, 
                n_hidden_1=n_hidden, 
                n_hidden_2=n_hidden, 
                ...
                dropout_rate1=dropout_rate1, 
                dropout_rate2=dropout_rate2,
                # Hier nutzen wir die dynamische Entscheidung:
                unpruned_neurons=eval_unpruned_neurons,
                pbt_layers=eval_pbt_layers
            ).to(device)
            
            eval_model.load_state_dict(best_model_state)
            
            try:
                log.info("Starte automatische Pruning-Struktur-Analyse (Heatmaps)...")
                heatmap_prefix = f"{artifact_file_prefix}_analysis" 
                pruning_connectivity_report = analyze_neuron_connectivity(
                    model=eval_model, 
                    model_name_prefix=heatmap_prefix,
                    output_dir=current_run_results_dir, 
                    logger_instance=log
                )
            except Exception as e_heatmap:
                log.error(f"Fehler bei Heatmap: {e_heatmap}")
            ...
            # ... (WandB Logging Code bleibt gleich) ...
        ...
        if run_fhe_pipeline and best_model_state:
            ...
            # Auch hier die dynamische Entscheidung nutzen!
            fhe_model = QATPrunedSimpleNet(
                input_size=input_size, 
                num_classes=num_classes, 
                n_hidden_1=n_hidden, 
                n_hidden_2=n_hidden, 
            ...
            fhe_model.load_state_dict(best_model_state)
            
            # Immer unprunen vor FHE (stellt sicher, dass Masken weg sind)
            fhe_model.unprune()
            fhe_model.cpu()
            
            # ... (Rest des FHE Codes bleibt gleich) ...

            # --- ANFANG ÄNDERUNG: AUTOMATISCHE HEATMAP-ERSTELLUNG ---
            try:
                log.info("Starte automatische Pruning-Struktur-Analyse (Heatmaps)...")
                # Der Prefix wird aus dem Artefakt-Namen extrahiert
                heatmap_prefix = f"{artifact_file_prefix}_analysis" 
                
                # Rufe die (oben definierte) Analysefunktion auf
                pruning_connectivity_report = analyze_neuron_connectivity(
                    model=eval_model, # Das gerade geladene beste Modell
                    model_name_prefix=heatmap_prefix,
                    output_dir=current_run_results_dir, # Speichere im selben Ordner
                    logger_instance=log
                )
                log.info("Pruning-Struktur-Analyse (Heatmaps) abgeschlossen.")
                
            except Exception as e_heatmap:
                log.error(f"Fehler bei der automatischen Erstellung der Pruning-Heatmap: {e_heatmap}")
                log.debug(traceback.format_exc())
            # --- ENDE ÄNDERUNG ---

            # (Die ursprüngliche Evaluierung läuft wie gewohnt weiter)
            torch_eval_res = evaluate_torch_model(eval_model, test_loader, device, label_encoder, model_name=f"{artifact_file_prefix}_pytorch_eval", results_dir=current_run_results_dir, n_samples=pytorch_eval_n_samples, logger=log)
        ...
        if run_fhe_pipeline and best_model_state:
            ...
            # Wir prüfen das gespeicherte State-Dict.
            # Fall A: Es enthält 'weight_mask' -> Wir müssen PBT beim Init aktivieren.
            # Fall B: Es enthält KEINE Masken (nur 'weight') -> Wir dürfen PBT NICHT aktivieren.
            
            pbt_active_in_state_dict = False
            for k in best_model_state.keys():
                if "weight_mask" in k:
                    pbt_active_in_state_dict = True
                    break
            
            # Entscheidung: Welche Parameter übergeben wir an den Konstruktor?
            if pbt_active_in_state_dict:
                log.info("State-Dict enthält Masken: Initialisiere FHE-Modell MIT PBT-Logik.")
                fhe_unpruned_neurons = unpruned_neurons_val
                fhe_pbt_layers = pbt_layers_val
            else:
                log.info("State-Dict ist clean (Masken eingebacken): Initialisiere FHE-Modell OHNE PBT-Logik.")
                fhe_unpruned_neurons = None  # Deaktiviert PBT im Konstruktor
                fhe_pbt_layers = []
            ...
            fhe_model = QATPrunedSimpleNet(
                ...
                dropout_rate1=dropout_rate1, 
                dropout_rate2=dropout_rate2,
                # Hier die dynamisch gewählten Werte nutzen:
                unpruned_neurons=fhe_unpruned_neurons,
                pbt_layers=fhe_pbt_layers
            )
            fhe_model.load_state_dict(best_model_state)
            fhe_model.unprune()
            ...
            if compile_sample_size > 0:
                ...
                # --- NEU: p_error aus Config lesen ---
                # Standard ist None (Library default ~ 2^-40), wenn nicht in YAML gesetzt
                desired_p_error = fhe_eval_cfg_params.get("p_error", None) 
                
                if desired_p_error:
                    log.info(f"Verwende benutzerdefinierten p_error: {desired_p_error}")
                else:
                    log.info("Verwende Standard p_error (sehr sicher, evtl. langsam).")

                start_compile_t = time.time()
                try:
                    # --- UPDATE: p_error übergeben ---
                    quant_numpy_module = compile_brevitas_qat_model(
                        fhe_model, 
                        compile_data,
                        p_error=desired_p_error  # <--- HIER EINFÜGEN
                    )
        ...
        log_data = {
            ...
            "model_details_runtime": {"layer_structure_generated": layer_structure, "dropout_rate1_applied": dropout_rate1, "dropout_rate2_applied": dropout_rate2, "quantization_bits_applied": cfg_qb_early, "n_hidden_applied": cfg_h_early},
            ...
            "best_model_metrics_achieved_val": {
                ...
                # --- KORREKTUR: Tippfehler (NameError) behoben ---
            ...
            # --- ANFANG ÄNDERUNG 2: JSON LOGGING DER GESAMT-SPARSITY ---
            # (Original-Zeile 868)
            "pruning_log_details": {
                # 'final_pruning_summary' IST BEREITS DIE LAYER-WISE AUFSCHLÜSSELUNG
                "final_sparsity_layerwise": final_pruning_summary, 
                
                # --- NEU: HINZUFÜGEN DER BERECHNETEN GESAMT-SPARSITY ---
                "overall_sparsity_info": {
                    "total_weights_combined": total_weights_combined,
                    "total_zeros_combined": total_zeros_combined,
                    "overall_sparsity_calculated": (
                        round(overall_sparsity_calculated, 5) 
                        if isinstance(overall_sparsity_calculated, float) 
                        else overall_sparsity_calculated
                    )
                },
                # --- ENDE NEUER LOG-EINTRÄGE ---
                
                # --- NEU: HINZUFÜGEN DES KONNEKTIVITÄTS-REPORTS ---
                "connectivity_analysis_report": pruning_connectivity_report,
                
                "pruned_layers_list": (list(torch_model.pruned_layers) if hasattr(torch_model, 'pruned_layers') and torch_model.pruned_layers else [])
            },
            # --- ENDE ÄNDERUNG 2 ---
        ...
        # --- (WandB Artefakt-Logging, Zeile 888-927, bleibt unverändert) ---
        if wandb_mode != "disabled" and wandb.run and wandb_cfg.get("log_artifacts", True):
            ...
            try:
                ...
                # --- NEU: Logge die Heatmaps zu WandB ---
                for layer_name in ['fc1', 'fc2', 'fc3']: # Annahme dieser Layer-Namen
                    heatmap_filename = f"{artifact_file_prefix}_analysis_{layer_name}_structured_mask_heatmap.png"
                    heatmap_path = os.path.join(current_run_results_dir, heatmap_filename)
                    if os.path.exists(heatmap_path):
                        wandb.log({f"pruning_heatmap_{layer_name}": wandb.Image(heatmap_path)})
                # --- ENDE NEUER WANDB-LOG ---
