# src/utils/structured_analyze_pruning_structure.py
...
def load_model_for_pruning_analysis(
    ...
    KORRIGIERTE VERSION: Diese Version ruft NICHT mehr die alte .prune()-Methode
    auf, die unstrukturiertes Pruning angewendet hat. Sie lädt das 
    State Dict direkt in die Basisarchitektur.
    ...
    try:
        ...
        # cfg_up_orig = model_config_params.get("unpruned_neurons", 0) # Nicht mehr verwendet
        ...
        # if cfg_up_orig > 0 and input_size > cfg_up_orig:
        #     logger_instance.info(f"  Modellinstanz wird gepruned (max_non_zero_per_neuron={cfg_up_orig}) um State Dict zu matchen.")
        #     model.prune(max_non_zero_per_neuron=cfg_up_orig)
        
        # Lade das State Dict. Dies funktioniert, weil das strukturierte Pruning
        # (prune.remove()) die Gewichte permanent ändert, aber nicht die
        # Architektur-Layer selbst (nur deren Inhalt).
        ...
        # Dieser Fehler wird erwartet, wenn das *alte* (unstrukturierte) Pruning
        # analysiert wird, da dort 'weight_mask' erwartet wird.
...
def analyze_neuron_connectivity(
    ...
    indem die Absolutwerte der Gewichte analysiert werden.
    ...
    # --- KORREKTUR: Analysiere Gewichte direkt, nicht Masken ---
    for name in layer_fc_names:
        ...
        # --- KORREKTUR: Analyse der Masken-Statistiken ---
        if current_mask_data is not None:
            ...
            logger_instance.info(f"    Davon komplett geprunt (Zeile ist Null): {dead_output_neurons} ({dead_output_neurons/num_output_neurons:.1%})")
            logger_instance.info(f"    Input-Features (Spalten) in diesem Layer: {num_input_features}")
            logger_instance.info(f"    Davon komplett geprunt (Spalte ist Null): {dead_input_features} ({dead_input_features/num_input_features:.1%})")
            
            # Alte (weniger nützliche) Statistik
            # active_weights_per_output_neuron = np.sum(current_mask_data, axis=1)
            # ...
...
if __name__ == "__main__":
    ...
    user_input_base_dir = input(
        f"Pfad zum Ordner '{DEFAULT_BASE_RESULTS_DIR_NAME}', spezifischen 'meta_run_*' Ordner, Condition-Ordner (z.B. 'qb5_p32') oder 'rep_X' Ordner angeben.\n" # Text angepasst
    ...
    # --- KORREKTUR: "rep_" zur Liste der gültigen Zielordner-Namen hinzugefügt ---
    is_direct_target_folder = (
        basename_of_search_dir.startswith("meta_run_") or
        basename_of_search_dir == "parameter_analysis_runs" or
        basename_of_search_dir.startswith("rep_") or # <-- HIER KORRIGIERT
        any(basename_of_search_dir.startswith(pfx) for pfx in ["CICIoT_qb", "EdgeIIoT_qb", "default_config", "qb"])
    )
    # --- ENDE KORREKTUR ---

    # Szenario 1: Der angegebene Pfad ist bereits ein 'rep_X'-Ordner oder ein anderer Zielordner
    if is_direct_target_folder:
        leaf_log_dirs_to_process.append(base_dir_to_search)
        log_main.info(f"Einzelner Lauf-Ordner ('{basename_of_search_dir}') wird analysiert.")
    else:
