...
# NEU: Suffix für die Re-Evaluierungsdatei
REEVAL_EXECUTE_SUFFIX: str = "_EXECUTE_REEVAL_execute.txt"
...
def parse_json_log(file_path: str,
    ...
    """
    Parst die Haupt-JSON-Log-Datei und extrahiert Konfigurations- und Ergebnisdaten.
    """
    try:
...
def parse_eval_text_file(file_path: str, prefix: str) -> Dict[str, Any]:
    """
    Parst eine Text-Evaluierungsdatei (PyTorch, FHE) und extrahiert Metriken.
    """
...
def process_repetition_folder(rep_folder_path: str,
    ...
    """
    Durchsucht einen Wiederholungs-Ordner nach Log-Dateien, parst diese
    und sammelt die Daten. Berücksichtigt jetzt auch die Re-Evaluierungsdatei.
    """
    aggregated_data: List[Dict[str, Any]] = []
    potential_run_subdirs = [
        d for d in os.listdir(rep_folder_path)
        if os.path.isdir(os.path.join(rep_folder_path, d)) and
           re.match(r"^\d{8}_\d{6}_", d) # Sucht nach Zeitstempel-Format JJJJMMDD_HHMMSS_
    ]
    ...
    for actual_log_containing_path in paths_to_process_logs_in:
        ...
        for json_file_path in json_log_files:
            if os.path.isfile(json_file_path):
                ...
                if data_item:
                    # Parse original PyTorch eval text file
                    ...
                    # Parse original FHE eval text files (simulate/execute)
                    ...
                    # --- ANFANG DER ERWEITERUNG ---
                    # Hinzugefügter Codeblock zum Parsen der Re-Evaluierungsdatei.
                    # Sucht nach der speziellen `_EXECUTE_REEVAL_execute.txt` Datei.
                    reeval_execute_txt_path = None
                    for file_name in os.listdir(actual_log_containing_path):
                        if file_name.endswith(REEVAL_EXECUTE_SUFFIX):
                            reeval_execute_txt_path = os.path.join(actual_log_containing_path, file_name)
                            # Verwendet ein neues, eindeutiges Präfix für die Spaltennamen, um Konflikte zu vermeiden.
                            reeval_prefix = "fhe_reeval_execute_txt"
                            reeval_txt_data = parse_eval_text_file(reeval_execute_txt_path, reeval_prefix)
                            data_item.update(reeval_txt_data)
                            # Geht davon aus, dass es nur eine solche Datei pro Ordner gibt und bricht die Schleife ab.
                            break 
...
def find_and_process_repetitions(base_path_for_condition: str,
    ...
    """
    Findet und verarbeitet alle Wiederholungs-Ordner ('rep_*') in einem Condition-Ordner.
    """
    aggregated_data: List[Dict[str, Any]] = []
...
def perform_overall_analysis(all_runs_data: List[Dict[str, Any]], base_results_dir_for_saving: str) -> Optional[pd.DataFrame]:
    ...
    Die neuen Spalten aus der Re-Evaluierung werden hier ebenfalls verarbeitet.
    """
    ...
    # --- ANFANG DER ERWEITERUNG ---
    # Liste der neuen Spaltennamen aus der Re-Evaluierungsdatei.
    new_reeval_cols = [
        'fhe_reeval_execute_txt_Acc', 'fhe_reeval_execute_txt_Precm', 'fhe_reeval_execute_txt_Recm',
        'fhe_reeval_execute_txt_F1m', 'fhe_reeval_execute_txt_F1w', 'fhe_reeval_execute_txt_MCC',
        'fhe_reeval_execute_txt_ROC-AUCm', 'fhe_reeval_execute_txt_total_infer_time_s_txt',
        'fhe_reeval_execute_txt_time_per_1000_samples_s_txt', 'fhe_reeval_execute_txt_fhe_compilation_time_s_txt',
    ]
    # --- ENDE DER ERWEITERUNG ---
    ...
    # Fügt die neuen Spalten zur Konvertierungsliste hinzu
    numeric_cols_to_convert.extend(new_reeval_cols)
    ...
    # --- ANFANG DER ERWEITERUNG ---
    # Fügt die neuen Re-Evaluierungsmetriken zur Aggregationsliste hinzu.
    metrics_to_aggregate.extend([
        'fhe_reeval_execute_txt_Acc', 'fhe_reeval_execute_txt_Precm', 'fhe_reeval_execute_txt_Recm',
        'fhe_reeval_execute_txt_F1m', 'fhe_reeval_execute_txt_F1w', 'fhe_reeval_execute_txt_MCC',
        'fhe_reeval_execute_txt_ROC-AUCm', 'fhe_reeval_execute_txt_fhe_compilation_time_s_txt',
    ])
    # --- ENDE DER ERWEITERUNG ---
    ...
    if not meta_run_folders:
        ...
        if os.path.isdir(base_results_dir):
            ...
            if potential_conditions:
                ...
                meta_run_folders = [base_results_dir]
