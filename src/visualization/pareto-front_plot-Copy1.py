...
def plot_separate_pareto_fronts(df_aggregated: pd.DataFrame):
    ...
    ax.plot(pareto_edge['fhe_time_mean'], pareto_edge['f1_mean'],
            color='red', linestyle='--', marker='D', zorder=5, label='Pareto-Front (EdgeIIoT)')
    ax.plot(pareto_ciciot['fhe_time_mean'], pareto_ciciot['f1_mean'],
            color='red', linestyle=':', marker='D', zorder=5, label='Pareto-Front (CICIoT)')

    # Versätze für xytext, um Labels leicht zu streuen und Überlappung zu vermeiden
    vertical_offsets = [0, 5, -5, 3, -3, 7, -7]
    offset_cycle = (vertical_offsets * ((len(df_aggregated) // len(vertical_offsets)) + 1))[:len(df_aggregated)]
    
    for i, (_, row) in enumerate(df_aggregated.iterrows()):
        dy = offset_cycle[i]
        dx = 6  # alle Labels nach rechts versetzt
    ...
    plt.title('F1-Score vs. FHE Inference Time with Pareto Fronts', fontsize=18)
    plt.xlabel('FHE Inference Time (s / 1000 samples) - Log Scale', fontsize=14)
    plt.ylabel('F1-Score (Weighted)', fontsize=14)
    plt.xscale('log')
    plt.grid(True, which="both", ls="--", alpha=0.5)
    ax.legend(title='Legende', fontsize=12)
...
if __name__ == '__main__':
    try:
        ...
        data_file_path = os.path.join(
            project_root, 'results', 'qat_runs', 'all_runs_detailed_data_20250526101416.csv'
        )
    except NameError:
        data_file_path = os.path.join(
            'results', 'qat_runs', 'all_runs_detailed_data_20250526101416.csv'
        )
