...
Enthält System-Monitoring (CPU/RAM), spezifische Formatierung und Einzelzeit-Messung.
"""
...
def evaluate_fhe_model(
    ...
    fhe_compilation_time_s: Optional[Union[float, str]] = None,
    ...
    # Initialer Aufruf, um Counter zu resetten (misst Zeit seit Start bis jetzt)
    ...
    for i in tqdm(range(len(X_subset)), desc=f"FHE {fhe_mode}", leave=False):
        ...
        try:
            ...
            duration = t_sample_end - t_sample_start
            sample_times.append(duration)
            
            # CPU Messung JETZT durchführen (für das Intervall der Inferenz)
    ...
    # Einzelzeiten String bauen
    sample_times_str = "\n=== Detailed Sample Times ===\n"
    for idx, t in enumerate(sample_times):
        sample_times_str += f"Sample {idx+1}: {t:.4f}s\n"
